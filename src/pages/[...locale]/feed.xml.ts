 ...
 * @returns The RSS feed for the Astra Browser release notes.
 */
export function GET(context: { url: URL }) {
  ...
  const rssData: RSSOptions = {
    title: 'Astra Browser Release Notes',
    description: 'Release Notes for the Astra Browser',
    site: context.url,
    items: [],
    customData: `
            <language>en</language>
            <link>https://astra-browser.app/release-notes</link>
            <copyright>Astra Browser © ${new Date().getFullYear()} - Made with ❤️ by the Astra team.</copyright>
            <pubDate>${pubDate(latestDate)}</pubDate>
            <image>
                <url>https://astra-browser.app/favicon.ico</url>
                <title>Astra Browser</title>
                <link>https://astra-browser.app</link>
            </image>
        `,
  }
  ...
  for (const releaseNote of releaseNotes.slice(0, RSS_ENTRY_LIMIT)) {
    rssData.items.push({
      ...
      link: `https://astra-browser.app/release-notes/${releaseNote.version}`,
...
function formatReleaseNote(releaseNote: ReleaseNote) {
  let content = `<p>
        If you encounter any issues, please report them on <a href="https://github.com/Hrishikeshmind/astradesktop/issues/">the issues page</a>.
...
function fixToReleaseNote(fix?: Exclude<ReleaseNote['fixes'], undefined>[number]) {
  ...
  if (fix.issue) {
    note += ` (<a href="https://github.com/Hrishikeshmind/astradesktop/issues/${fix.issue}" target="_blank">#${fix.issue}</a>)`
  }
  return note
}
